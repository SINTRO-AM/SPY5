...
import time
from SPY3_logic import preparing_df, get_date_today, SMA_signal, VaR_signal, Rebound_signal, VaR_threshold
import math
import yaml
import os
...
def main():
    ...
    price_bond = get_price("SHY")  # Angenommen, du hast eine Methode, um den Preis für PASSIVE_SYMBOL zu bekommen

    app_positions = app.positions.get(acc_ID)
    if not app_positions:
        app_positions = app.positions
    print(app_positions)
    if signal: #if signal == True then buy future Risk On!!
        if (ACTIVE_SYMBOL in app_positions) : #& (app_positions[ACTIVE_SYMBOL]["position"] >0)
            print("Risk On! I would buy now.")
            ...
            if quantity > 0:
                app.place_trade(ACTIVE_SYMBOL, 'BUY', quantity, "MKT")
            ...
            quantity = math.floor(available_funds / price_spy)
            if quantity > 0:
                app.place_trade(ACTIVE_SYMBOL, 'BUY', quantity,"MOC") ## MOC - muss noch angepasst werden.
       ...
       # Continuing with the trading logic for the PASSIVE_SYMBOL signal
        ...
        if ACTIVE_SYMBOL in app_positions:
            ...
            if quantity > 0:
                app.place_trade(PASSIVE_SYMBOL, 'BUY', quantity, "MOC")
        elif not app_positions:
