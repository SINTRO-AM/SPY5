...
#Input
start_date = dt.datetime(1999, 6, 1)
...
conditions = [
    ((df['VaR_1d'] < 0.05) & ((df['30D_MA'] > df['200D_MA']) | (df['VaR_1d'] < 0.02) | (df['Fwd'] < -0.15))),  # Alle Bedingungen zutreffend
    (df['VaR_1d'] < 0.05) & ((df['30D_MA'] < df['200D_MA']) & (df['VaR_1d'] > 0.02) & (df['Fwd'] > -0.15)),  # Nur VaR_1d < 0.05 zutreffend
    (df['VaR_1d'] >= 0.05)  # Keine der Bedingungen zutreffend
]

# Definiere die Werte, die jeweils zugewiesen werden sollen
choices = [
    1,  # Signalwert, wenn alle Bedingungen zutreffend
    -1, # Signalwert, wenn nur VaR_1d < 0.05 zutreffend
    0   # Signalwert, wenn keine Bedingungen zutreffend
]

# Wende die Bedingungen an und weise die Werte zu
df['Signal'] = np.select(conditions, choices, default=np.nan)

# Überprüfe, ob es irgendwelche NaN-Werte gibt, die behandelt werden müssen
# Wenn ja, fülle sie wie benötigt (hier als Beispiel mit 0)
df['Signal'].fillna(0, inplace=True)

    
# SignalB
df['SignalB'] = np.where(df['Signal'].isin([-1, 0]),  1, 1)
...
def generate_shapes_for_signals(df):
    ...
    for i, row in df.iterrows():
        if row['Signal'] in [0,-1] and not in_shape:
            start_date = row['Date']
            in_shape = True
